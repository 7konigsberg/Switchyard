...
/// Session-state key counting strong-tier turns served since the latch (de-escalation only).
const STRONG_CALLS_KEY: &str = "escalation_strong_calls";
/// Session-state key counting consecutive release verdicts (de-escalation only).
const RELEASE_STREAK_KEY: &str = "escalation_release_streak";
/// Session-state key counting efficient turns still exempt from judging after a hard-limit return.
...
struct EscalationClassifier {
    ...
    /// Reversible routing: judges strong-tier responses and releases the session back.
...
/// De-escalation settings with the strong-phase judge that reviews the strong tier's replies.
...
impl EscalationClassifier {
    /// Serves a latched turn on the strong tier and, once the minimum hold is met, judges its
    /// reply; enough consecutive release verdicts clear the latch for the next request.
    async fn review_capable(
        ...
        if capable_response.served_model() == Some(efficient) {
            // A weak fallback during the strong phase is not a strong reply: do not judge it
            // as one, and do not let it extend a release streak.
        ...
        let agg = match capable_response.llm_response.into_agg().await {
            ...
            Err(source) => {
                return Err(LibsyError::client_call(capable.clone(), source));
            }
        };
        ...
        let capable_response = Response {
            ...
            upstream_headers: capable_response.upstream_headers,
        };
        ...
        Ok((decisive(capable), Some(capable_response)))
    }
}
...
impl Classifier<State> for EscalationClassifier {
    async fn score(
        ...
        if self.deescalation.is_none() && streak(state) >= self.confirmations {
        ...
        if weak_cooldown > 0 {
            ...
            return Ok((decisive(&efficient), Some(efficient_response)));
        }
...
mod tests {
    ...
    use crate::algorithms::util::escalation::DeescalationConfig;
    ...
    /// Builds a router with de-escalation enabled (`confirmations=1` latches immediately).
